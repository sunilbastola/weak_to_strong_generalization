...
from weak_to_strong.common import get_tokenizer, supports_bf16, get_device
...
def should_use_model_parallel():
    """
    Determines if model parallelism should be used based on available CUDA device properties.
    Only applicable for CUDA devices; returns False for other devices.
    """
    device = get_device()
    if device == "cuda":
        try:
